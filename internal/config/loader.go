...
type rawServiceEntry struct {
	...
	Inline *Service // inline service definition (when value is a JSON object)
}
...
func (lc *loadContext) loadRootFile(path string) (*Config, error) {
	...
	for name, entry := range raw.Services {
		if entry.Inline != nil {
			cfg.Services[name] = entry.Inline
			continue
		}
...
// ServiceFragment is the schema for nested config files.
...
// loadFragment loads a single .json5 file as a ServiceFragment, merges its
